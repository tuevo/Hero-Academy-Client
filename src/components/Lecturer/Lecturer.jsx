...
function Details(props) {
  ...
  const [
    openRemoveAccountConfirmDialog,
    setOpenRemoveAccountConfirmDialog,
  ] = useState(false);
      ...
      <Box p={6} className={classes.details}>
        ...
        <Box py={2} display='flex' flexDirection='column' alignItems='center'>
          ...
          <Box display='flex' alignItems='center'>
            <Typography
              variant='body2'
              style={{ marginRight: 3 }}
            >{`${Math.floor(data.roleInfo.averageRating)}.${
            ...
            <Box style={{ marginBottom: 1 }}>
              <Rating
                name='read-only'
                value={data.roleInfo.averageRating}
                size='small'
                precision={0.5}
                readOnly
              />
          ...
          <Box>
            <Typography variant='body2'>
              <span>(</span>
              <NumberFormat
                value={data.roleInfo.numberOfRatings}
                displayType={'text'}
                thousandSeparator={'.'}
                decimalSeparator={','}
        ...
        <Box py={4}>
          ...
          <Box pb={2} display='flex' justifyContent='space-between'>
            ...
            <Typography variant='body2' color='textPrimary'>
              <NumberFormat
                value={data.roleInfo.numberOfStudents}
                displayType={'text'}
                thousandSeparator={'.'}
                decimalSeparator={','}
              />
...
export default function Lecturer({ data }) {
  ...
  return (
    <div>
                ...
                <Grid item xs={4}>
                  <Box position='relative'>
                    ...
                    <Box
                      ...
                      alignItems='center'
                      className={classes.details__numberOfRatings}
                    >
                      <StarIcon
                        style={{ color: '#ffb600', fontSize: '0.875rem' }}
                      />
                      <Typography
                        variant='body2'
                        color='inherit'
      ...
      <Details onClose={handleClose} data={data} open={open} />
    </div>
  );
}
